...
export const fsWatch: NotiTool = {
  name: "fs_watch",
  description:
    "Watch a directory inside the workspace for file changes for a fixed duration, then return the events observed (change/rename). Blocks for duration_ms and returns. Recursive watching is best-effort (fully supported on macOS/Windows).",
  schema: z.object({
  ...
  handler: async (args, ctx) => {
    const dir = await resolveWorkspacePath(ctx, args.path ?? ".");
